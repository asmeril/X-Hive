"""
Test thread posting with XHive (bypass rate limit)
"""
import asyncio
import json
import sys
...
async def test_thread_bypass():
    daemon = XDaemon()
    await daemon.start()
    
    # Bypass rate limit for testing
    daemon.rate_limiter.operations = {}  # Clear rate limit history
...
if __name__ == "__main__":
    asyncio.run(test_thread_bypass())